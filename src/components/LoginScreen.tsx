...
export const LoginScreen: React.FC<LoginScreenProps> = ({ onLogin }) => {
  ...
  return (
    <div className={`relative min-h-screen bg-[#0B101E] text-white flex items-center justify-center p-4 overflow-hidden font-sans selection:bg-rose-600 selection:text-white select-none transition-all duration-1000 ${
      isTransitioning ? 'scale-150 opacity-0 filter blur-md' : 'scale-100 opacity-100'
    }`}>
      {/* Background Animated Concentric Labyrinth Rings */}
      ...
      <div className={`relative z-10 w-full max-w-md bg-[#1A2235]/80 backdrop-blur-xl border border-slate-700/60 rounded-3xl p-8 shadow-2xl shadow-rose-950/40 space-y-6 transition-all duration-700 ${
        ...
        <div className="space-y-4 pt-2">
          <div>
            ...
            <div className="grid grid-cols-2 gap-2 p-1 bg-[#0B101E] rounded-xl border border-slate-800">
              <button
                type="button"
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full bg-[#0B101E] border border-slate-800 rounded-xl px-3.5 py-2.5 text-xs text-slate-200 font-mono focus:outline-none"
              />
        ...
        <div className="space-y-3 pt-2">
          ...
          >
            <span>{isTransitioning ? 'Entering ChakraView Portal...' : `Login as ${selectedRole === 'analyst' ? 'Bank Analyst' : 'Police Admin'}`}</span>
